...
function FAQItem({
  ...
  return (
    ...
    >
      ...
      >
        <span className={`font-sans text-sm md:text-base font-medium leading-snug transition-colors duration-200 ${isOpen ? "text-brand-black" : "text-brand-black/80"}`}>
          {question}
        </span>
      ...
      {/* Accordion CSS — max-height transition, zero JS */}
      <div
        id={`faq-answer-${index}`}
        role="region"
        style={{
      ...
      >
        <div className="px-6 pb-5 pt-0">
          ...
          <p className="font-sans text-sm text-brand-gray-text leading-relaxed">{answer}</p>
        </div>
      </div>
    </div>
  );
...
export default function FAQSection() {
  ...
  return (
    <section id="duvidas" className="section-padding bg-white" aria-labelledby="faq-titulo">
      <div className="container-default">
        <div className="grid lg:grid-cols-5 gap-8 lg:gap-12">

          {/* Texto lateral */}
          <div className="lg:col-span-2 flex flex-col gap-6">
            <SectionTitle eyebrow="Dúvidas" title="Perguntas frequentes" align="left" />
